...
public class SeasonalPlotReservation extends Reservation<SeasonalPlot> {
	...
	public SeasonalPlotReservation(UserAccount user, SeasonalPlot product, LocalDateTime begin, LocalDateTime end, PayMethod payMethod) {
	...
	public MonetaryAmount getTotalPrice(double electricity, double water) {
		if(PayMethod.MONTHLY.equals(payMethod)){
			return getProduct().getPrice().add(getProduct().settlementElectricity(electricity).add(getProduct().settlementWater(water)));
		}
