...
|Registered User| um customer zu werden muss sich der User registrieren und wird im System als Registered User mit geringsten privilegien angelegt|
|Cart| Stellplätze müssen im wahrenkorb liegen um reserviert zu werden|
|Reservation|eine Reservierung hat ein An- und Abreise-Datum und einen Status|
|Status| findet am Anreisetag kein Checkin statt (ARRIVED) wird die Reservierung ungültig (CANCELLED)|
|Item|neben Stellplätzen können auch die Optionen Sportgeräte und Parkplätze gebucht werden|
|Inventory|enthält alle Items|
|Cataloge|enthält alle verfügbaren bzw. ausgewählte Items|
|Boss|hat alle Zugriffsrechte: im Dashboard kann er seine Umsätze und Buchungen überwachen|
|Employee|hat keinen zugriff auf das Dashborad, aber auf alles Andere. Er kann auch buchungen für Kunden erstellen, Reservierungen als ARRIVED markieren, oder Reservierungen löschen|
|===
...
|Ergebnis     a|Er bekommt eine Liste mit allen Sportgeräten angezeigt
|===
...
// das wird ja in der Aufgabenstellung erwähnt aber ich weiß nicht wie es je dazu kommen könnte
[cols="1h, 4"]
[[TAE02]]
|===
|ID            |<<TAE02>>
...
|Ergebnis     a|Das Sportgerät word auf der {glossar_website} als ausgeliehen angezeigt
|===
...
|Ergebnis     a|Das Sportgerät wird auf der {glossar_website} wieder als verfügbar angezeigt
|===
...
|Ergebnis     a|- Der _Preis_ des {glossar_plot}es wird aktualisiert und angezeigt
|===
...
|Ergebnis     a|Der neue {glossar_plot} wird in dem {glossar_system} gespeichert
|===
...
|Erwartung    a|Ein {glossar_employee} nutzt die Seite
|Event        a|Der {glossar_employee} drückt in dem <<PlatzKatalog>> auf erstellen und füllt ein Formular aus
|Ergebnis     a|Es wird eine Fehlermeldung angezeigt die aussagt, dass dieser {glossar_plot} bereits existiert
|===
...
|Ergebnis     a|Der {glossar_plot} wird aus dem {glossar_system} entfernt und ist nicht mehr vorhanden
|===
...
|Anwendungsfall|<<DatenAnpassen>>
|Erwartung    a|Ein {glossar_employee} nutzt die Seite
|Event        a|{glossar_employee} drückt auf dem <<SportgeräteKatalog>> bei einem Sportgerät auf bearbeiten und legt folgende neue Werte fest:

- _Preis_: 12

Anschließend drückt er auf "Informationen aktualisieren"
|Ergebnis     a|Der _Preis_ des Sportgerätes wird aktualisiert und angezeigt
|===
...
|Anwendungsfall|<<DatenAnpassen>>
|Erwartung    a|Ein {glossar_employee} nutzt die Seite
|Event        a|Der {glossar_employee} drückt in dem <<SportgeräteKatalog>> auf erstellen und füllt ein Formular aus
|Ergebnis     a|Das neue Sportgerät wird in dem {glossar_system} gespeichert
|===
...
|Erwartung    a|Ein {glossar_employee} nutzt die Seite
|Event        a|Der {glossar_employee} drückt in dem <<SportgeräteKatalog>> auf erstellen und füllt ein Formular aus
|Ergebnis     a|Es wird eine Fehlermeldung angezeigt die aussagt, dass dieses Sportgerät bereits existiert
|===
...
|Ergebnis     a|Das Sportgerät wird aus dem {glossar_system} entfernt und ist nicht mehr vorhanden
|===
...
|Ergebnis     a|Es wird eine Seite mit diversen Informationen über den Campingplatz angezeigt
|===
