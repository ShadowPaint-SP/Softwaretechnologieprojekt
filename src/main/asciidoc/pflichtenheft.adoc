...
|Event        a|{glossar_employee} markiert die <<PlatzReservieren, Reservierung>> vom {glossar_customer} als wahrgenommen
|Ergebnis     a|
- Es wird ein Fehler angezeigt weil der vom kunden reservierte {glossar_plot} nichtmehr vorhanden ist
|===
...
|Ergebnis     a|Der {glossar_plot} wird aus dem {glossar_system} entfernt und ist nichtmehr vorhanden
|===
...
|Ergebnis     a|Das Sportgerät wird aus dem {glossar_system} entfernt und ist nichtmehr vorhanden
|===
